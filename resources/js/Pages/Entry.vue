<template>
  ...
  <CenteredLayout 
    ...
    subtitle="Masukkan kode survey untuk mengikuti survey"
    max-width="max-w-md"
  >
     <!-- Token Input Form -->
     <form @submit.prevent="submitToken" class="space-y-6">
       ...
       <div class="form-control">
         ...
         <div class="relative">
           <input 
             v-model="form.survey_code" 
             type="text" 
             placeholder="Masukkan kode survey Anda" 
             class="pr-12 w-full input input-bordered"
             :class="{ 'input-error': form.errors.survey_code }"
             required
           />
     ...
     <div class="mt-6 alert alert-info">
       ...
       <div>
         ...
         <div class="mt-1 text-xs">
           Kode survey diberikan oleh administrator survey. Pastikan Anda memasukkan kode yang benar untuk mengakses survey yang dituju.
         </div>
       </div>
     </div>
...
const route = (name) => {
  const routes = {
    ...
    'survey.respondent-data': '/survey/respondent-data'
  }
  return routes[name] || '/'
}
...
const submitToken = () => {
  form.post(route('survey.enter'), {
    onSuccess: (page) => {
      // Backend returns JSON response with token and redirect URL
      const data = page.props || page
      if (data.success) {
        // Store survey token in localStorage for client-side access
